...
(() => {
  ...
  function form(title,fields,save,help="",saveLabel="Save"){
    if(activeDialog?.isConnected)return;
    const d=n('dialog',{class:'work-dialog','aria-label':title}),inputs={};activeDialog=d;
    d.append(n('h2',{text:title}));if(help)d.append(n('p',{class:'ws-sub',text:help}));
    ...
    d.append(n('div',{class:'ws-actions'},[btn(saveLabel,async()=>{const values=Object.fromEntries(Object.entries(inputs).map(([k,x])=>[k,x.value]));try{await save(values,d);d.close();d.remove();activeDialog=null;await refresh(true);}catch(e){note(d,e);}}),btn('Cancel',()=>{d.close();d.remove();activeDialog=null;})]));
  ...
  function route(source){const c=current(),rooms=[...window.__atlas.state.rooms.values()];const suggested=(source.sources||[]).map(r=>r.room||r.vault||r.label).find(id=>rooms.some(r=>r.id===id));
    form('Choose a room for this idea',[{key:'room',label:'Which room?' ,options:rooms.map(r=>[r.id,r.title||r.id]),value:suggested||c.room},{key:'reason',label:'Anything you want the agent to consider?' ,long:true,value:source.explanation?.why||source.why||'Would this help what we are doing here?' }],async v=>{
  ...
  async function signal(post,article){
    ...
    try{
      ...
      function paint(source){slot.replaceChildren();article.hidden=source.disposition==='dismissed'||source.snooze_until>Date.now()/1000;
        ...
        if(explanation && explanation.source_digest===source.digest){
          const heading=article.querySelector('h3'),body=article.querySelector('.tell-post-body');
          explained=true;if(heading)heading.textContent=explanation.title;
          if(body){body.replaceChildren(n('p',{text:explanation.idea}),n('p',{},[n('strong',{text:'Why it matters: '}),n('span',{text:explanation.why})]),n('p',{},[n('strong',{text:'A small next step: '}),n('span',{text:explanation.next})]));}
        }
  ...
  function renderOverview(){if(!all)return;const host=document.getElementById('tellBody');if(!host)return;
    let toolbar=host.querySelector('#workOverview');if(!toolbar){toolbar=n('details',{id:'workOverview',class:'work-overview'},[n('summary',{text:'Saved ideas & results'})]);host.append(toolbar);}
    const open=toolbar.open;toolbar.replaceChildren(n('summary',{text:'Saved ideas & results'}));toolbar.open=open;
    const m=all.metrics;toolbar.append(n('p',{text:`${m.interested} saved · ${m.awaiting_review} waiting for a room · ${m.helped} helped · ${m.failed} did not help`}),n('p',{class:'ws-sub',text:all.coverage}));
    for(const s of all.sources.filter(s=>s.interest||s.disposition==='dismissed'||s.snooze_until>Date.now()/1000||all.experiments.some(e=>e.source===s.id))){const routes=all.routes.filter(r=>r.source===s.id);const row=n('article',{class:'work-card'},[n('strong',{text:s.explanation && s.explanation.source_digest===s.digest?s.explanation.title:s.title}),n('p',{class:'ws-sub',text:routes.map(r=>r.room+': '+r.status).join(' · ')||'Not assigned to a room'})]);
